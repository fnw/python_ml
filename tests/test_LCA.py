...
from python_ml.Ensemble.Selection.Dynamic.DCS import OLA, LCA
...
class test_ola(unittest.TestCase):
    def setUp(self):
        ...
        np.random.seed(5)
        self.predictions = np.random.randint(
            low=np.min(self.y),
            high=np.max(self.y),
            size=(self.n_queries, self.n_neighbors, self.n_classifiers),
        )
        self.predictions_query = np.random.randint(
            low=np.min(self.y),
            high=np.max(self.y),
            size=(self.n_queries, self.n_classifiers),
        )
    ...
    def test_local_class_accuracy(self):
        _, actual_labels = self.selector.predict_nearest(
            self.X[: self.n_queries, :], self.bag, voting_scheme=None
        )

        a = self.selector.best_local_class_accuracy(
            self.predictions, self.predictions_query, actual_labels
        )
